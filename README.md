...
| 윤혜인 | `대중교통 접근성` <br> `보행안전` <br> `주차시설` | 1) 서울특별시 버스정류소 위치정보 (2020), 공공자전거 대여소 정보 (2020), <br>&emsp;서울교통공사 역주소현황(행정동기준) (2020) <br> 2) 서울시 주차장 확보율 통계 (2020) <br> 3) 서울시 보행환경 만족도 통계 (2019) <br>
| 이기쁨 | `공원` <br> `교육환경` <br> `청결도` | 1) 서울시 공원 (1인당 공원면적) 통계 (2019) <br> 2) 서울시 평생교육기관 (유형별구별) 통계 (2018), <br>&emsp;서울시 신고·등록 체육시설 통계 (2019), 서울의 도서관 (2021) <br> 3) 서울시 생활안전도 통계 (2018) 
| 정아영 | `소음` <br> `의료시설` <br> `치안 및 방범`  | 1) 서울시 소음진동민원 현황 통계 (2010~2018)<br> 2) 서울시 의료기관 (구별) 통계 (2019), 서울특별시 응급실 위치 정보 (2020)<br> 3) 서울시 자치구 목적별 cctv 설치 현황 (2018), <br>&emsp;서울시 시내주요기관(경찰·소방관서) 통계 (2019)
...
전체 1인가구에서 청년 1인가구가 차지하는 비중이 가장 높지만, 2021년 1월 한겨레 기사(*청년 1인가구 65%는 월세로 산다…공공임대 입주는 1%대*) 에 따르면 청년 1인가구 10명 중 3명은 주거비로 월 소득의 30%이상을 지출하는 주거비 과부담 가구이다. 

월 소득 대비 임대료 비율을 나타내는 PIR이 30%를 초과하면 주거비 과부담 가구로 보는데, 이 비중이 청년 1인 가구는 31.4%로 일반가구(26.7%)나 1인가구 평균(30.8%)보다 높았다. 이는 초기 자산이 없어 보증금이 낮고 월세가 높은 형태로 거주하기 때문인 것으로 분석되어, 청년 1인가구의 주거비 과부담 비율이 높은 이유가 공공임대 공급 물량 부족과 관련된 것으로 분석되었다.
...
<a name="preprocessing"> <a/>
 ...
 소주제 | 설명 
  --- | ---
 문화시설 | 영화관 수, 민간·공공 공연장
 상업시설 | 교육 서비스업, 도매 및 소매업, 숙박 및 음식점업, 공공행정 국방 및 사회보장 행정,  예술 스포츠 및 여가관련 서비스업
 대중교통 접근성 | 공공자전거(따릉이), 서울시내버스, 지하철 데이터
 보행안전 | 횡단보도, 교통사고, 보행안전 통계 데이터
 주차시설 | 주차장 확보율 : (주차면수/자동차대수)x100
 공원 및 녹지 | 1인당 공원면적
 교육환경 | 도서관 수(대표(중구), 국립(강남,서초,영등포구), 공공도서관(그 외)의 수의 합계), 평생학습관의 수, 체육시설 수
 청결도 | 대기오염, 수질오염, 길거리의 쓰레기방치에 대한 주민 설문 조사
 소음 | 환경 관련 전체민원 중 소음진동 민원 건수 (2010~2018년도 평균)
 의료시설 | 응급실 현황, 병원 수
 치안 및 방범 | 경찰서, 전체 CCTV 현황 중 방범용 CCTV 수

- **자치구 칼럼명 통일**   
 자치구는 모든 데이터에서 공통으로 사용하는 칼럼이기 때문에 칼럼명을 통일시켜줌
 ```python
 data.rename(columns={'구별': '자치구'}, inplace=True)
 ```   
...
- **불필요한 열 삭제**   
 데이터 통합을 위해 불필요한 칼럼은 `drop`
 
- **인구대비 데이터로 통일**   
 모든 데이터는 해당 데이터 연도의 '자치구별 인구수'로 나누어 인구대비 데이터로 만들어줌 (`서울시 주민등록인구 (구별) 통계` 데이터 사용)
...
- 각 주제 변수들 간의 상관관계를 보면 `공원면적-문화시설`, `의료기관-치안방범`, `문화시설-대중교통`, `상업시설-주차시설`, `보행안전-주차시설`, `대중교통-주차시설` 관계에서 **강한 양적 선형관계(0.7~1.0)** 를 보여주고 있다. 
- 또한 부정적인 영향도인 `청결도`와 `소음민원`에서 다른 변수들과 음의 상관관계를 많이 보인다는 것을 볼 수 있고, 특히 `청결도` 항목에서 `문화시설`, `상업시설`, `보행안전`, `주차시설`, `대중교통`의 항목들과 **강한 음적 선형관계(-1.0~-0.7)** 를 보여주고 있다.
...
- 이후 minmaxscaler 방식을 이용하여 점수의 구간을 (0,1)로 맞춰주는 작업을 진행했다.
