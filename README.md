...
소음의 경우에만 특정구가 독보적인 최대값을 가지고, 연도별 추이가 다른 것으로 나타났다. 이에, 컬럼 항목별 수치를 합산하여 선정하는 방식으로 결정했는데, 항목 특성상 감점 요인으로 고려되어지는 '소음'과 '청결도'는 점수를 마이너스화 한 후 진행하였다.

항목별 수치의 범위도 다르고, 자치구별 인구수도 크게 차이가 나기 때문에 모든 항목을 더하여 총 합계를 구한 후, 가장 최신 자치구별 인구수 데이터를 수집하여 인구대비 합계 컬럼을 생성했다. 
이후, minmaxscale방식을 이용하여 점수의 구간 (0,1)로 맞춰주는 작업을 진행했다.
