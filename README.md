...
`이기쁨`  | `공원` <br> `교육환경` <br> `청결도` | 1) 서울시 공원 (1인당 공원면적) 통계 (2019) <br> 2) 서울시 평생교육기관 (유형별구별) 통계 (2018), 서울시 신고·등록 체육시설 통계 (2019), 도서관의 수 (2021) : 대표(중구) + 국립(강남,서초,영등포구) + 공공도서관(그 외) / https://lib.seoul.go.kr/slibsrch/main <br> 3) 서울시 생활안전도 통계 (2018) 
